...
- [HAProxy](https://www.haproxy.org/) (опционально). TLS-edge на одном порту, если MDM и Remote на одном VPS
...
- ОС Ubuntu, возможные варианты:
    - Ubuntu Noble `24.04 LTS` (рекомендуется в этом форке)
    - Ubuntu Jammy `22.04 LTS` (рекомендуется в этом форке)
...
Систему необходимо загрузить на сервер посредством `git`:
...
Краткая инструкция по Ubuntu 22.04/24.04 и совместному хостингу с MDM: [INSTALL-UBUNTU.md](./INSTALL-UBUNTU.md).
...
Если на том же хосте Headwind MDM уже занимает публичный `:443`:
...
Теперь вы можете зайти в веб-приложение администратора (web-admin) по адресу `https://remote.example.com/web-admin/` (добавьте `:9443`, если задали свой HTTPS-порт).

### Один VPS с Headwind MDM (публичный 443)

HAProxy на хосте позволяет MDM и Remote делить `:443` с реальным IP клиента в Devices:
...
Подробности: [scripts/single-port/README.md](./scripts/single-port/README.md).
